...
export default function WalletPage({ params }: { params: { id: string } }) {
  ...
  const essentialWallet = api.essentialWallet.getOne.useQuery({
    id: params.id ?? "",
  });

  const receiverWalletDetails = api.openPayments.getWalletDetails.useQuery(
    {
      walletAddress: essentialWallet.data?.data?.walletAddress ?? "",
    },
    { enabled: false },
  );
  ...
  const incomingPayment = api.openPayments.createIncomingPayment.useQuery(
    {
      walletAddress: senderWalletAddress,
      receiverAddress: essentialWallet.data?.data?.walletAddress ?? "",
      value: donation,
    },
    { enabled: false },
  );
  ...
  useEffect(() => {
    ...
    console.log(senderWalletDetails.data?.data)
  }, [senderWalletAddress]);
          ...
          <div className="grid grid-cols-6 items-center justify-center gap-6 md:grid-cols-12 md:gap-4">
            <div className=" col-span-6 md:col-span-4">
              <Image
                ...
                shadow="md"
                src={essentialWallet.data?.data?.imageUrl ? essentialWallet.data.data.imageUrl : ""}
                width="100%"
              />
            </div>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex flex-col gap-0">
                  <h1 className="font-semibold text-foreground/90">
                    {essentialWallet.data?.data?.name}
                  </h1>
                  <h3 className="text-foreground/80">
                    Current Balance: R{essentialWallet.data?.data?.balance}
                  </h3>
                  <Snippet symbol="" color="primary">
                    {window.location.href}
                  </Snippet>
                  <p className="text-md mt-2 ">{essentialWallet.data?.data?.about}</p>
                </div>
              </div>
              ...
              <div className="mt-3 flex flex-col gap-4">
                ...
                <Input
                  ...
                  value={essentialWallet.data?.data?.walletAddress}
                  color="danger"
                  disabled
                />
                <Input
